...
struct cli_config initial_control_socket_config = {
  .name = PATH_CONTROL_SOCKET,
  .mode = 0660,
};

struct cli_config initial_yi_control_socket_config = {
  .name = NULL,
  .mode = 0600,
};
...
yi_connect(sock *s, uint size UNUSED)
{
  ...
  s->tx_hook = cli_tx;
  s->err_hook = cli_err;
  s->data = c = yi_cli_new(s, ((struct cli_listener *) s->data)->config);
  s->pool = c->pool;
  s->fast_rx = 1;
  c->rx_pos = c->rx_buf;
  rmove(s, c->pool);
...
yi_init_unix(uid_t use_uid, gid_t use_gid)
{
  ...
  log("yi inited, tbuf %i", yi_control_socket->s->tbuf);
}
...
char *opt_list = "bc:dD:ps:Y:P:u:g:flRh";
int parse_and_exit;
char *bird_name;
char *use_user;
char *use_group;
