...
void
sk_log_ao_info(sock *s, const char *name)
{
  struct ao_info info;
  ...
  log(L_DEBUG "%s: TCP-AO on socket fd %i: current key %i, rnext key %i, good packets %lu, bad packets %lu",
      name, s->fd, info.current_key, info.rnext_key, info.pkt_good, info.pkt_bad);
}

void
sk_log_ao_keys(sock *s, const char *name)
{
  struct tcp_ao_getsockopt_ext *keys;
  int keys_num;
  ...
  log(L_DEBUG "%s: TCP-AO on socket fd %i has %i keys", name, s->fd, keys_num);
  for (int i = 0; i < keys_num; i++)
  {
    ...
    log(L_DEBUG "Key id %i/%i: %s%salgo %s, secret %s",
	key->sndid, key->rcvid, key->is_current ? "current, " : "",
