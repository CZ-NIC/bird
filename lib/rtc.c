...
rtc_parse(u64 type, u32 asn, struct f_val asn_ip, u64 val, int pxlen, enum rtc_prefix_position px_pos)
{
  ...
  /*
   * In cases when the end of the prefix does not lie directly at the boundary
   * of RTC fields, prefix length is specified relative to this particular field.
   * In order to calculate pxlen of the entire RTC prefix, we have to add length
   * of preceding fields.
   *
   * 1. User did not enter valid RTC prefix, just 64-bit value. Do nothing.
   ...
   * 6. There is no prefix, pxlen is useless.
   */
