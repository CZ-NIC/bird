...
#define NB_RT_FILTER	(1 << NET_RT_FILTER)
...
typedef struct net_addr_rt_filter {
  u8 type;
  u8 pxlen;
  u16 length;
  u32 asn;
...
#define NET_ADDR_RT_FILTER(asn,rt,pxlen) \
  ((net_addr_rt_filter) { NET_RT_FILTER, .pxlen = pxlen, .length = sizeof(net_addr_rt_filter), .asn = asn, .rt = rt })
...
static inline void net_fill_rt_filter(net_addr *a, u32 asn, vpn_rt rt, u32 pxlen)
{ *(net_addr_rt_filter *)a = NET_ADDR_RT_FILTER(asn, rt, pxlen); }
...
static inline int net_is_rt_filter(const net_addr *a)
{ return (a->type == NET_RT_FILTER); }
...
static inline int net_equal_rt_filter(const net_addr_rt_filter *a, const net_addr_rt_filter *b)
{ return !memcmp(a, b, sizeof(net_addr_rt_filter)); }
...
static inline int net_zero_rt_filter(const net_addr_rt_filter *a)
...
static inline int net_compare_rt_filter(const net_addr_rt_filter *a, const net_addr_rt_filter *b)
...
static inline void net_copy_rt_filter(net_addr_rt_filter *dst, const net_addr_rt_filter *src)
{ memcpy(dst, src, sizeof(net_addr_rt_filter)); }
...
static inline u32 net_hash_rt_filter(const net_addr_rt_filter *n)
...
static inline int net_validate_rt_filter(const net_addr_rt_filter *n)
