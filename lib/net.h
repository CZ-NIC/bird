...
#define NET_ADDR_RTC(asn,rt,pxlen) \
  ((net_addr_rtc) { NET_RTC, pxlen, sizeof(net_addr_rtc), asn, rt })
...
static inline void net_fill_rtc(net_addr *a, u32 asn, vpn_rt rt, u32 pxlen)
{ *(net_addr_rtc *)a = NET_ADDR_RTC(asn, rt, pxlen); }
...
static inline int net_is_rtc(const net_addr *a)
{ return (a->type == NET_RTC); }
...
static inline uint net_vlan_id(const net_addr *a)
{
  ASSERT_DIE(a->type == NET_ETH);
  return ((net_addr_eth *) a)->vid;
}
...
static inline int net_equal_rtc(const net_addr_rtc *a, const net_addr_rtc *b)
{ return !memcmp(a, b, sizeof(net_addr_rtc)); }
...
static inline int net_zero_rtc(const net_addr_rtc *a)
{ return !a->pxlen && vrt_zero(a->rt); }
...
static inline int net_compare_rtc(const net_addr_rtc *a, const net_addr_rtc *b)
{ return uint_cmp(a->asn, b->asn) ?: vrt_compare(a->rt, b->rt); }
...
static inline void net_copy_rtc(net_addr_rtc *dst, const net_addr_rtc *src)
{ memcpy(dst, src, sizeof(net_addr_rtc)); }
...
static inline u32 net_hash_rtc(const net_addr_rtc *n)
{ return hash_value(vrt_hash0(n->rt, HASH_PARAM, u32_hash(n->asn))); }
...
static inline int net_validate_rtc(const net_addr_rtc *n)
{
  return (n->asn > 0 || n->asn == 0) && (n->pxlen <= 64) &&
	 ((vrt_to_u64(n->rt) & ~u64_mkmask(n->pxlen)) == 0);
}
