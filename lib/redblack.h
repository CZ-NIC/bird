...
#define REDBLACK_CHECK(type, name, root, compare) do { \
  if (!root) \
    break; \
  type *prev = NULL; \
  ...
  while (pos > 0) { \
	...
	if (REDBLACK_LEFT_CHILD(name, stack[pos].node)) { \
	  ...
	  ASSERT(compare(stack[pos+1].node, stack[pos].node) < 0); \
	  pos++; \
	  continue; \
	} \
	stack[pos].state++; \
	__attribute__((fallthrough)); \
      case 1: \
	ASSERT(!prev || (compare(prev, stack[pos].node) < 0)); \
...
#define REDBLACK_FIND_POINTER(name, root, compare, what, pointer) \
  for ( \
      int _cmp = !(pointer = &(root)); \
      (*pointer) && (_cmp = compare((what), (*pointer))); \
      pointer = &(REDBLACK_CHILD(name, (*pointer), ((_cmp < 0) ? 1 : 2))) \
      )

#define REDBLACK_FIND(type, name, root, compare, what) \
  ({ type **pointer; REDBLACK_FIND_POINTER(name, root, compare, what, pointer); *pointer; })
...
#define REDBLACK_INSERT(type, name, root, compare, what) do { \
  type **where = &(root); \
  what->name[1] = what->name[2] = NULL; \
  REDBLACK_FIND_POINTER(name, root, compare, what, where) \
