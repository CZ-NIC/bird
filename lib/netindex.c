...
static void NETINDEX_REHASH(callback *cb) {
  SKIP_BACK_DECLARE(netindex_spinhash, v, rehash_cb, cb);
...
netindex_hash_new(pool *sp, struct birdloop *cleanup_target, u8 type)
{
  ...
  callback_init(&nh->cleanup_cb, netindex_hash_cleanup, cleanup_target);
...
{
  SKIP_BACK_DECLARE(struct netindex_hash_private, nh, cleanup_cb, cb);
...
netindex_hash_delete(netindex_hash *h, event *e, event_list *t)
{
  ...
  callback_activate(&hp->cleanup_cb);
}
