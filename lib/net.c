...
const u16 net_addr_length[] = {
  ...
  [NET_FLOW4]	= 0,
  [NET_FLOW6]	= 0,
  [NET_RT_FILTER] = sizeof(net_addr_rt_filter),
...
const u16 net_max_text_length[] = {
  ...
  [NET_FLOW4]	= 0,	/* "flow4 { ... }" */
  [NET_FLOW6]	= 0,	/* "flow6 { ... }" */
  [NET_RT_FILTER] = 38,	/* "4294967296:4294967296/128 AS4294967295" */
...
STATIC_ASSERT(sizeof(net_addr_rt_filter)== 16);
...
STATIC_ASSERT(alignof(net_addr_flow4)	== alignof(net_addr));
STATIC_ASSERT(alignof(net_addr_flow6)	== alignof(net_addr));
STATIC_ASSERT(alignof(net_addr_rt_filter)== alignof(net_addr));
...
net_format(const net_addr *N, char *buf, int buflen)
{
  ...
  case NET_RT_FILTER:
    {
    struct vpn_rd rd = {
      .hi = n->rt_filter.rt.hi,
      .lo = n->rt_filter.rt.lo,
    };

    int c = rd_format(rd, buf, buflen);
    ADVANCE(buf, buflen, c);
    return bsnprintf(buf, buflen, "/%u AS%u", n->rt_filter.pxlen, n->rt_filter.asn);
    }
  case NET_IP6_SADR:
...
net_compare(const net_addr *a, const net_addr *b)
{
  ...
  case NET_RT_FILTER:
    return net_compare_rt_filter((const net_addr_rt_filter *) a, (const net_addr_rt_filter *) b);
  case NET_IP6_SADR:
...
net_hash(const net_addr *n)
{
  ...
  case NET_RT_FILTER: return NET_HASH(n, rt_filter);
...
net_validate(const net_addr *n)
{
  ...
  case NET_RT_FILTER: return NET_VALIDATE(n, rt_filter);
