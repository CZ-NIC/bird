...
msl_alloc_from_page(mslab *s, struct msl_head *h)
{
  ...
  for (; ti->used_bits_index < s->head_bitfield_len; ti->used_bits_index++)
  {
	...
	return NULL;
      ...
      ti->used_bits_local[ti->used_bits_index] = ti->used_bits_local[ti->used_bits_index] | (1 << pos);
...
msl_refresh_bitfield(struct mslab *s, struct msl_head *head)
{
  ...
  {
    /* We can't assert here that (~ti->used_bits_local[i] == 0)
     * because at the last index the bitfield may be shorter */
