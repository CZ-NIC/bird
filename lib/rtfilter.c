...
#include "nest/bird.h"
#include "lib/net.h"
#include "lib/rtfilter.h"

int
rtc_format(char *buf, int buflen, const net_addr_rtfilter *n)
{
  ...
  /*
   *  Route target constraint has one of three different formats:
   *
   *  0           16         32           64
   *  |     2B     |    2B    |     4B     |
   *  +------------+----------+------------+
  ...
  /* Position of boundary (in bits) between ASN/IP address and value */
  int boundary = 48;
  ...
  else if (type == 0x0202)
  {
    ...
    /* ASN doesn't fit into 2 bytes, we can write "rt" instead of "rt-as4" without ambiguity */
    if (asn > 0xffff)
      type_short = "rt";
  }
