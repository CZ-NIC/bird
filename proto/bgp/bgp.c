...
bgp_start(struct proto *P)
{
  ...
  {
    p->rtc_tree = NULL;
    p->rtc_tree_pool = lp_new(p->p.pool);
    p->rtc_initial_feed = true;

    fib_init(&p->rtc_fib, p->p.pool, NET_RTC, sizeof(struct fib_node), 0, 0, NULL);
...
bgp_shutdown(struct proto *P)
{
  ...
  if (p->cf->rtc_use)
  {
    settle_cancel(&p->rtc_settle);
    fib_free(&p->rtc_fib);

    p->rtc_tree = NULL;
    p->rtc_tree_pool = NULL;
    p->rtc_fib = (struct fib) { 0 };
    p->rtc_settle = (struct settle) { 0 };
    p->rtc_settle_cf = (struct settle_config) { 0 };
  }
...
struct protocol proto_bgp = {
  ...
  .channel_mask =	NB_IP | NB_VPN | NB_FLOW | NB_EVPN | NB_MPLS | NB_NEIGHBOR | NB_RTC,
