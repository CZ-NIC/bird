		     ...
		     struct bgp_ao_key *backup, int current_key_id, int rnext_key_id)
{
  ...
  if (key->key.send_id == current_key_id)
  {
  ...
  if (key->key.recv_id == rnext_key_id)
  {
...
bgp_reconfigure_ao_keys(struct bgp_proto *p, const struct bgp_config *cf)
{
  ...
  if (!EMPTY_LIST(old_keys))
  {
    ...
    info.backup = bgp_select_best_ao_key(p);
...
bgp_show_proto_info(struct proto *P)
{
  ...
  else if (P->proto_state == PS_UP)
  {
    ...
    if (!EMPTY_LIST(p->ao.keys))
    {
      ...
      cli_msg(-1006, "    TCP-AO:");
      cli_msg(-1006, "      Current key Send ID: %i", info.current_key);
      cli_msg(-1006, "      RNext key Recv ID: %i", info.rnext_key);
