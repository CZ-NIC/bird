...
bgp_use_gateway(struct bgp_export_state *s)
{
  ...
  /* Check for non-matching AF */
  if (!bgp_channel_match_next_hop_af(c, nhad->nh.gw))
    return NULL;
...
static void
bgp_decode_evpn_ead(struct bgp_parse_state *s, net_addr_evpn *net, byte *pos, uint len, ea_list **a)
{
  if (len < (8+10+4+3))
    bgp_parse_error(s, 1);
  ...
  bgp_apply_mpls_labels(s, a, 1, &label);
...
static void
bgp_decode_evpn_mac(struct bgp_parse_state *s, net_addr_evpn *net, byte *pos, uint len, ea_list **a)
{
  if (len < (8+10+4+7+1))
    bgp_parse_error(s, 1);
  ...
  bgp_apply_mpls_labels(s, a, lnum, label);
...
static void
bgp_decode_evpn_imet(struct bgp_parse_state *s, net_addr_evpn *net, byte *pos, uint len, ea_list **a UNUSED)
{
  if (len < (8+4+1))
    bgp_parse_error(s, 1);
...
static void
bgp_decode_evpn_es(struct bgp_parse_state *s, net_addr_evpn *net, byte *pos, uint len, ea_list **a UNUSED)
{
  if (len < (8+10+1))
    bgp_parse_error(s, 1);
...
static void
bgp_decode_evpn_unknown(struct bgp_parse_state *s, net_addr_evpn *net, uint type, byte *pos, uint len, ea_list **a UNUSED)
{
  /* Store raw data */
...
static void
bgp_decode_nlri_evpn(struct bgp_parse_state *s, byte *pos, uint len, struct ea_list *base_attrs)
{
  ...
  while (len)
  {
    ...
    switch (type)
    {
    case NET_EVPN_EAD:	bgp_decode_evpn_ead(s, net, pos, rlen, &ea); break;
    case NET_EVPN_MAC:	bgp_decode_evpn_mac(s, net, pos, rlen, &ea); break;
    case NET_EVPN_IMET:	bgp_decode_evpn_imet(s, net, pos, rlen, &ea); break;
    case NET_EVPN_ES:	bgp_decode_evpn_es(s, net, pos, rlen, &ea); break;
    default:		bgp_decode_evpn_unknown(s, net, type, pos, rlen, &ea); break;
    }
...
bgp_create_update_bmp(ea_list *channel_ea, byte *buf, byte *end, struct bgp_bucket *buck, bool update)
{
  ...
  struct bgp_write_state s = {
    ...
    .direct_ip4_next_hop = (BGP_AFI(desc->afi) == BGP_AFI_IPV4) || (BGP_AFI(desc->afi) == BGP_AFI_L2VPN),
    .desc = desc,
    .mpls = desc->mpls,
...
again:
  ...
  struct bgp_write_state s = {
    ...
    .direct_ip4_next_hop = bgp_channel_is_ipv4(ptx->c) || bgp_channel_is_l2vpn(ptx->c),
    .desc = c->desc,
    .mpls = c->desc->mpls,
  };
