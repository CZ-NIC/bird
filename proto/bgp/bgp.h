...
#define BGP_AF_RTC		BGP_AF( BGP_AFI_IPV4, BGP_SAFI_RTC )
...
struct bgp_proto {
  ...
  struct f_tree  *rtc_tree;
  struct linpool *rtc_tree_pool;
  struct fib rtc_fib;
  struct settle rtc_settle;
  struct settle_config rtc_settle_cf;
  bool rtc_initial_feed;
};
...
void bgp_build_rtc_tree_on_settle(struct settle *s);
void bgp_process_rtc_entry(struct bgp_proto *p, const struct net_addr_rtc *addr, const struct rta *a);
