...
static void
l3vpn_generate_route(struct l3vpn_proto *p, const struct net_addr_rtc *addr)
{
  struct rta a0 = {
  ...
  rte_update2(p->rtc_channel, (net_addr *)addr, e, p->p.main_source);
}
...
l3vpn_split_interval(struct l3vpn_proto *p, const struct f_tree *node)
{
  ...
  while (current <= end)
  {
    ...
    struct net_addr_rtc addr = NET_ADDR_RTC(0, vrt_from_u64(current), 64 - order);
...
l3vpn_init(struct proto_config *CF)
{
  ...
  proto_configure_channel(P, &p->rtc_channel, proto_cf_find_channel(CF, NET_RTC));
...
l3vpn_reconfigure(struct proto *P, struct proto_config *CF)
{
  ...
  if (import_changed)
  {
    ...
    if (p->rtc_channel)
    {
      rt_refresh_begin(p->rtc_channel->table, p->rtc_channel);
      l3vpn_split_interval(p, p->import_target);
      rt_refresh_end(p->rtc_channel->table, p->rtc_channel);
    }
...
struct protocol proto_l3vpn = {
  ...
  .channel_mask =	NB_IP | NB_VPN | NB_MPLS | NB_RTC,
