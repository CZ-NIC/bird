...
static void
channel_stop_export_req(struct channel *c, struct rt_export_request *req, event *ev)
{
...
{
  channel_stop_export_req(c, &c->out_req, &c->out_event);
  channel_stop_export_req(c, &c->alt_req, &c->alt_event);
}
