...
static _Atomic int proto_rethink_goal_pending;
...
protos_do_commit(struct config *new, struct config *old, int type)
{
  ...
  if (old)
  {
	...
	p->cf_new = NULL;
      ...
      atomic_fetch_add_explicit(&proto_rethink_goal_pending, 1, memory_order_acq_rel);
  ...
  if (!atomic_load_explicit(&proto_rethink_goal_pending, memory_order_acquire))
    protos_do_commit(new, old, type);
}
...
proto_cleanup(struct proto *p)
{
  ...
  if (1 == atomic_fetch_sub_explicit(&proto_rethink_goal_pending, 1, memory_order_acq_rel))
  {
