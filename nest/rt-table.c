...
static void
rte_do_update(struct rte_update_event *rue)
{
  struct channel *c = rue->channel;
  const net_addr *n = rue->net;
  rte *new = rue->rte;
  struct rte_src *src = rue->src;
...
static void
rte_dispatch_update(struct rte_update_event *rue)
{
  rte_do_update(rue);
}
