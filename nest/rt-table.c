...
rte_update_commit(struct rte_update_batch *batch, struct channel *c)
{
  ...
  for (struct rte_update *ru = batch->first; ru; ru = ru->next)
    if (!ru->src)
      if (ru->rte)
	if (ru->rte->attrs->src)
	  ru->src = ru->rte->attrs->src;
	else if (!rta_is_cached(ru->rte->attrs))
	  ru->src = ru->rte->attrs->src = c->proto->main_source;
	else
	  bug("src is NULL in cached RTA");
      else
	ru->src = c->proto->main_source;

  if (c->in_table)
    for (struct rte_update *ru = batch->first; ru; ru = ru->next)
      if (!rte_update_in(c, ru->n, ru->rte, ru->src))
	ru->flags |= RUF_IGNORE;
