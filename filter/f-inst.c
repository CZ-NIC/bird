...
/* Relational operators */
  ...
  INST(FI_NEXTHOP_APPLY, 1, 0) {
    NEVER_CONSTANT;
    ACCESS_RTE;

    eattr *heea = ea_find(fs->rte->attrs, &ea_gen_hostentry);
    if (!heea)
      runtime("Failed to apply nexthop, no hostentry in the route");

    nexthop_apply(fs->rte, (struct hostentry_adata *) heea->u.ptr);
  }
